...
CRUST_INLINE Crust_HashIndexMap crustHashIndexMapInit(void *context, const Crust_HashIndexMapVtbl *vtbl, usize key_size, usize key_alignment)
{
	...
	CRUST_ASSERT(crustIsPow2us(key_alignment) != 0);
	...
	result.key_stride = crustAlignUpus(key_size, key_alignment);
...
CRUST_INLINE void crustHashIndexMapFreeStorage(Crust_Allocator allocator, Crust_HashIndexMap *hash_map)
{
	...
	usize masks_capacity = crustAlignUpus(hash_map->capacity, 32) / 32;
...
CRUST_INLINE u32 crustSlotPoolCheck(const Crust_SlotPool *pool, Crust_SlotHandle handle)
{
	...
	if (crustSlotPoolCheckMask(pool, handle.index) == 0)
		return 0;
